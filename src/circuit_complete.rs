...
use halo2_proofs::{
    ...
    plonk::{Circuit, ConstraintSystem, Error, Selector, Advice, Column, Expression, Instance},
    poly::Rotation,
};
    ...
    fn configure(cs: &mut ConstraintSystem<F>) -> Self::Config {
        ...
        cs.create_gate("anemoi linear layer", |meta| {
            ...
            vec![
                ...
                s.clone() * (x0_after.clone() - x0_before.clone() - generator.clone() * x1_before.clone()),
                // x1' = generator * x0 + (generator^2 + 1) * x1
                s.clone() * (x1_after.clone() - generator.clone() * x0_before.clone() - generator_square_plus_one.clone() * x1_before.clone()),
                
                // MDS矩阵变换 for Y: y0' = y1 + generator * y0
                s.clone() * (y0_after.clone() - y1_before.clone() - generator.clone() * y0_before.clone()),
                // y1' = generator * y1 + (generator^2 + 1) * y0
                s * (y1_after.clone() - generator.clone() * y1_before.clone() - generator_square_plus_one.clone() * y0_before.clone()),
            ]
        });
        ...
        cs.create_gate("anemoi sbox", |meta| {
            ...
            vec![
                ...
                s.clone() * (x0_step1.clone() - x0_before.clone() + generator.clone() * y0_squared.clone()),
                s.clone() * (x1_step1.clone() - x1_before.clone() + generator.clone() * y1_squared.clone()),
                ...
                s.clone() * (x0_after.clone() - x0_step1.clone() - generator.clone() * y0_after.clone() * y0_after.clone() - generator_inv.clone()),
                s * (x1_after.clone() - x1_step1.clone() - generator.clone() * y1_after.clone() * y1_after.clone() - generator_inv.clone()),
            ]
        });
    ...
    fn synthesize(
        ...
        let (leaf_cell, root_cell) = layouter.assign_region(
            ...
            |mut region| {
                ...
                // 最终根验证
                config.root_verification_selector.enable(&mut region, offset)?;
        ...
        *offset += 1;
        ...
        for round in 0..14 {
            ...
            *offset += 1;
            ...
            // === 状态混合阶段 (PHT) ===
            config.anemoi_state_mixing_selector.enable(region, *offset)?;
            ...
            *offset += 1;
            ...
            jive_x[0] = x0_step1 + constants.generator * jive_y[0].square() + constants.generator_inv;
            jive_x[1] = x1_step1 + constants.generator * jive_y[1].square() + constants.generator_inv;
...
/// 用于外部计算和验证
pub fn complete_anemoi_jive_hash<F: PrimeField>(
    left: F,
    middle: F,
    right: F,
    padding: F,
